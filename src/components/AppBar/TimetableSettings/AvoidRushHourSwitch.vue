...
const avoidRushHour = ref(store.avoidRushHour ?? false);

watch(avoidRushHour, (val) => {
    if (val !== store.avoidRushHour) {
        store.avoidRushHour = val;
    }
});

watch(() => store.avoidRushHour, (val) => {
    if (val !== avoidRushHour.value) {
        avoidRushHour.value = val;
    }
});
</script>
