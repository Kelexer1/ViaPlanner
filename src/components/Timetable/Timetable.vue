    ...
    <div class="col-11 w-full pr-8">
      ...
      <div class="grid" name="weekDaysAxis">
        <div v-for="(weekday, index) in weekdays" :key="weekday" class="col">
          <WeekdaySwitch :weekday="weekday" :weekdayLabel="useShortWeekdays ? weekdaysShort[index] : weekday"
            :semester="semester" class="pb-[20px]" />
        </div>
      </div>
      <!-- Timetable Content -->
          ...
          <div v-for="event in getEventsForDay(meetingSections) as Array<any>"
            ...
            <template v-if="!event.isEmpty">
              <TimetableEvent v-for="(courseActivityData, index) in event.courses"
                :key="courseActivityData.course + courseActivityData.activity + index" :eventData="courseActivityData"
                :semester="semester" :day="day" :isEmpty="false"
