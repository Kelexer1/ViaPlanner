  ...
  <div class="flex flex-row h-full">
    ...
    <div class="col-11 w-full pr-8">
      ...
      >
        ...
        >
          <div
            v-for="hour in timeRange.length"
            :key="hour"
            :style="{
          ...
          <div
            ...
            class="absolute left-0 right-0 flex pb-[1px]"
            :style="{
...
const oneHourHeight = computed(() => {
  if ((height.value - 168) / 9 > 65) {
    return `${Math.round((height.value - 168) / 9)}px`;
  }

  return `65px`;
});

function getEventsForDay(meetingSections) {
  const result = [];
